...
- 插件可选声明 `library.artwork_candidates`，按 Host 传入的 `route:<routeKey>` / `branch:<nodeKey>` 作用域只返回稳定媒体 ID 与连接绑定的 Host asset UUID；真实图片由 Server 重新鉴权、限流、限制解码尺寸并合成为签名同源风格 3 分类封面。静态 `libraryArtwork` 始终用于插件最外层入口，并作为分类上游不可用时的兜底。
