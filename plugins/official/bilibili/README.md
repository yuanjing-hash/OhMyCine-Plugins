...
当前实现提供真实扫码登录、推荐/热门/排行/番剧/影视/纪录片、搜索、详情、分 P/UGC 合集/追更分集、收藏、稍后再看、历史、关注与追更读取、幂等远端动作、播放进度回传、progressive 与 DASH 双轨多清晰度播放、WebVTT 字幕、标准弹幕轨道、真实 DownloadPlan 和插件专属 ProviderMetadata。Server 与 Player 核心没有 Bilibili API 特判。
...
连接配置由声明式设置页提供，包含账号状态、首页推荐、默认清晰度、字幕、弹幕和元数据说明。默认清晰度仅在用户没有显式选择时生效。
