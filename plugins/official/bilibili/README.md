...
连接配置由声明式设置页提供，包含账号状态、首页推荐、默认清晰度、字幕、弹幕和元数据说明。默认清晰度仅在用户没有显式选择时生效。插件通过 `library.artwork_candidates` 把真实推荐视频海报注册成短时 Host asset，Server 负责筛选、解码限制、合成和签名分发；插件包内的原创横向封面仅作上游不可用时的兜底。Player 不直接读取插件文件、Host asset UUID 或 Bilibili 远端图片。
