...
- The official install source and source-of-truth is the dedicated `https://github.com/yuanjing-hash/OhMyCine-Plugins` multi-plugin repository. Its default branch contains plugin source, SDK, the installable Registry and clean-build publishing workflows. Each official Manifest `source`, Manifest URL, package URL and Release asset must resolve to that same repository; Player and Server repositories must link here instead of keeping a second publishable Registry.
